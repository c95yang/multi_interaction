...
class HOIDataset:
    ...
    def get_sequence(self, sbj: str, obj: str, act: str) -> h5py.Group:
        #print(f"Getting sequence for {sbj}, {obj}, {act}")

        #return self.h5dataset[sbj][f"{obj}_{act}"]

        sequence_key = f"{obj}_{act}"
        if sequence_key not in self.h5dataset[sbj]:
            available_sequences = list(self.h5dataset[sbj].keys())
            #print(f"Warning: Sequence '{sequence_key}' not found for subject '{sbj}'. Available sequences: {available_sequences}")
            return None
        return self.h5dataset[sbj][sequence_key]
    ...
    def _get_sbj2objact_from_split(self) -> Dict[str, List[str]]:
        ...
        for sbj, obj_act in split:
            #print(f"Processing object/action {sbj}, {obj_act}")
            _oa_split = obj_act.split("_")
            obj = _oa_split[0]
            #if len(_oa_split) > 2:
            act = "_".join(_oa_split[1:])
            #act = _oa_split[1]
            #print(f"Processing {sbj}, {obj}, {act}")
    ...
    def _load_data(self) -> List[H5DataSample]:
        ...
        for sbj, obj_acts in self.sbj2objact.items():
            for (obj, act) in obj_acts:
                #print(f"Loading data for {sbj}, {obj}, {act}")
