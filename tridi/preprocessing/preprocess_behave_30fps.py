...
def preprocess(cfg):
    ...
    #print(f"preposess_behave.py 42. Loaded sequences: {_sequences}")
    ...
    if cfg.behave.split in ["train", "test"]:
        ...
        print(f"preprocess_behave.py 52. Filtered sequences: {sequences}")
    ...
    for sequence in tqdm.tqdm(sequences, total=len(sequences), ncols=80):
        ...
        for k, v in smpl_params.items():
            print(f"preprocess_behave_30fps.py 145. smpl_params key: {k}, shape: {v.shape}")
            if k != "save_name" and k != "gender":
